...
func TestConfig_Next(t *testing.T) {
	cases := []struct {
		...
		{
			name:          "expansion rate < 0, nothing all value should be 0 - with capacity",
			current:       WithCapacity(1000, 5),
			expansionRate: -1,
			expected:      config{},
		},

		{
			name:          "expansion rate < 0, nothing all value should be 0 - with accuracy",
		...
		{
			name:          "expansion rate = 1, the next config would be the same - with capacity",
			current:       WithCapacity(1000, 5),
			expansionRate: 1,
			expected:      config{mode: "capacity", k: 5, storageCapacity: 1000},
		},
		...
		{
			name:          "expansion rate > 1, the next config would be calculated and round to uint32 - with capacity",
			current:       WithCapacity(1234, 5),
			expansionRate: 1.345,
			expected:      config{mode: "capacity", k: 5, storageCapacity: 1660},
		},
		...
		{
			name:          "expansion rate > 1, maximum would be max uint32 - with capacity",
			current:       WithCapacity(5000, 5),
			expansionRate: 1000000,
			expected:      config{mode: "capacity", k: 5, storageCapacity: math.MaxUint32},
		},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			c, ok := tc.current.(config)
			if !ok {
				t.Errorf("%v is not instance of config", c)
			}
			expected, ok := tc.expected.(config)
			if !ok {
				t.Errorf("%v is not instance of config", expected)
			}

			r := c.Next(tc.expansionRate)
			result, ok := r.(config)
			if !ok {
				t.Errorf("%v is not instance of config", r)
			}

			if &result == &c {
				t.Errorf("return value is the same instance of current config")
			}
			assertConfigEqual(t, result, expected)
		})
	}
}
